...
{
    public partial class Room_13 : Form
    {
        public Room_13()
        {
            InitializeComponent();
            if (GlobalBoss.reset == true)
            {
                ...
                picEmerald.Visible = true;
                picBronze.Visible = false;
                GlobalBoss.reset = false;
            }
        }

        private void Room_13_KeyDown(object sender, KeyEventArgs e)
        {
            if (picSilver.Visible == false && picGold.Visible == false && picEmerald.Visible == false)
            {
            ...
            if (lblExit.Bounds.IntersectsWith(playerImage.Bounds) && lblExit.Visible == true)
            {
                this.Hide();
        ...
        //prevents image from passing through walls/enemy 
        {
            foreach (Control x in this.Controls)
            {
                if (x is PictureBox && (string)x.Tag == "walls" && x.Visible == true)
                {
                    ...
                    {
                        playerImage.Location = new Point(392, 480);
                    }
                }
            }
            foreach (Control y in this.Controls)
            {
                if (y is PictureBox && (string)y.Tag == "wealth")
                {
                    if (playerImage.Bounds.IntersectsWith(y.Bounds) && y.Visible == true)
                    {
                        ...
                        else if (playerImage.Bounds.IntersectsWith(picGold.Bounds))
                        {
                            GlobalBoss.score = GlobalBoss.score + 4;
                        }
                        else if (playerImage.Bounds.IntersectsWith(picEmerald.Bounds))
                        {
