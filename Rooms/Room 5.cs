...
{
    public partial class Room_5 : Form
    {
        public Room_5()
        {
            InitializeComponent();
            if (GlobalBoss.reset == true)
            {
                ...
                picSilver4.Visible = true;
                picSilver5.Visible = true;
                picBronze.Visible = false;
                GlobalBoss.reset = false;
            }
        }

        private void Room_5_KeyDown(object sender, KeyEventArgs e)
        {
            if (picSilver1.Visible == false && picSilver2.Visible == false && picSilver3.Visible == false && picSilver4.Visible == false && picSilver5.Visible == false)
            {
            ...
            if (lblEast.Bounds.IntersectsWith(playerImage.Bounds) && lblEast.Visible == true)
            {
                ...
                if (playerImage.Top > (lblEast.Width - 60))
                {
                    playerImage.Top += 40;
                }
            }
            ...
            {
                this.Hide();
                Rooms.Room_10 room10 = new Rooms.Room_10();
                room10.Show();
            }
            if (lblWest.Bounds.IntersectsWith(playerImage.Bounds) && lblWest.Visible == true)
            {
                this.Hide();
                Rooms.Room_6 room6 = new Rooms.Room_6();
                room6.Show();
            }
        ...
        //prevents image from passing through walls/enemy 
        {
            foreach (Control x in this.Controls)
            {
                if (x is PictureBox && (string)x.Tag == "walls" && x.Visible == true)
                {
                    ...
                    {
                        playerImage.Location = new Point(543, 571);
                    }
                }
            }
            foreach (Control y in this.Controls)
            {
                if (y is PictureBox && (string)y.Tag == "wealth")
                {
                    if (playerImage.Bounds.IntersectsWith(y.Bounds) && y.Visible == true)
                    {
                        ...
                        else if (playerImage.Bounds.IntersectsWith(picSilver4.Bounds))
                        {
                            GlobalBoss.score = GlobalBoss.score + 2;
                        }
                        else if (playerImage.Bounds.IntersectsWith(picSilver5.Bounds))
                        {
                            GlobalBoss.score = GlobalBoss.score + 2;
                        }
                        y.Visible = false;
