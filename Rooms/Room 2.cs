...
namespace Maze.Rooms
{
    public partial class Room_2 : Form
    {
        ...
        private void Room_2_KeyDown(object sender, KeyEventArgs e)
        {
            if (GlobalBoss.hydreigonDefeated == true)
            {
                picHydreigonBoss.Visible = false;
                ...
                {
                    MessageBox.Show("This path is blocked! Find a different path");
                    if (playerImage.Left < (lblWest.Width +20))
                    {
                        playerImage.Left += 60;
                    }
                }
        ...
        {
            if (playerImage.Bounds.IntersectsWith(picHydreigonBoss.Bounds))
            {

                if (GlobalBoss.hydreigonDefeated == false)
                {
                    playerImage.Top = playerImage.Top + 40;
                    playerImage.Left = playerImage.Left + 40;
                    Boss_interactions.frmBossHydreigon hydreigon = new Boss_interactions.frmBossHydreigon();
                    hydreigon.ShowDialog();
                }
            }
